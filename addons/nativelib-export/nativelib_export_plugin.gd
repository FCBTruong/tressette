...
class NativeLibExportPlugin:
	...
	func _ios_add_bundles() -> void:
		var dir = DirAccess.open('res://addons/nativelib-export/iOS')
		if dir != null:
			dir.list_dir_begin()
	...
	func _ios_add_standard_frameworks() -> void:
		var f = FileAccess.open('res://addons/nativelib-export/iOS/std_frameworks.txt', FileAccess.READ)
			...
			while file_name != '':
				if not dir.current_is_dir() and file_name.ends_with('.plist'):
					if file_name == 'GoogleService-Info.plist':
						...
						var f = FileAccess.open(plist, FileAccess.READ)
	...
	func _process_hooks(hooks_path: String, args: Array) -> void:
		var dir = DirAccess.open(hooks_path)
		if dir != null:
			dir.list_dir_begin()
			var file_name = dir.get_next()
			while file_name != '':
