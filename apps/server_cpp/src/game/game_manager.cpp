...
void GameManager::on_login_success(uint64_t uid) {
    ...
    general_pkg.set_time_thinking_in_turn(config_.value("time_thinking_in_turn", 0));
    general_pkg.set_timestamp(timestamp_now);
    general_pkg.set_fee_mode_no_bet(config_.value("fee_mode_no_bet", 0));
    general_pkg.set_enable_ads(true);

    if (config_.contains("exp_levels") && config_["exp_levels"].is_array()) {
        for (const auto& exp_level : config_["exp_levels"]) {
            general_pkg.add_exp_levels(exp_level.get<int>());
        }
    }

    if (config_.contains("level_rewards") && config_["level_rewards"].is_array()) {
        for (const auto& reward : config_   ["level_rewards"]) {
